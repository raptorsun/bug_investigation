...
patch_node_exporter() {
    # disable netclass collector
    local overrides
    overrides=$(kc get daemonset node-exporter -o json | jq '.spec.template.spec.containers[0].args|=.+["--no-collector.netclass"]' | jq '{spec: .["spec"]}' | gojsontoyaml)
